...
We report three findings. First, local perturbations propagate through organisms with measurable temporal structure: lagged cross-correlations between spatially separated probes reach r = 0.84 (lag = 5 steps, n = 40 timepoints), with correlation strength decreasing with distance. Second, we observe emergent NAND-like behavior arising from self-repair dynamics: organisms survive single perturbations (~15% local mass removal) but collapse when two simultaneous perturbations exceed the repair threshold. "Collapse" is defined as mass falling below 25 (vs baseline ~73) within 200 steps. Across 80 trials (20 per input condition, randomized seeds), survival rates were: [0,0] 100%, [0,1] 100%, [1,0] 100%, [1,1] 0%. Comparing single-input (01/10 pooled, n=40) vs double-input (11, n=20) survival, Fisher's exact test yielded p = 2.4×10⁻¹⁶. Robustness held under position jitter (±3 px; 90% vs 25% survival, n=20) and amplitude variation (±20%; 95% vs 5%, n=20). Third, using Lenia dynamics as a reservoir with ridge regression readout, we classified XOR inputs correctly across 16 trials with randomized initializations (mean accuracy 100%, baseline linear classifier: 50%).

As NAND is functionally complete, these results suggest a pathway toward richer computation in self-organizing continuous cellular automata via damage-threshold dynamics near criticality. Key limitations include the binary survive/collapse output and absence of demonstrated cascading between gates.
...
- "Decreasing trend" = Spearman correlation between probe index and correlation magnitude is negative (ρ = -0.68, p = 0.09)
